...
export async function restoreVersion(
  ...
  return await withLock(`git:${resolve(repoDir)}`, async () => {
    ...
    const tmpPath = `${filePath}.ctxnest-restore-${process.pid}-${Date.now()}`;
...
export async function syncGlobalVault(
  ...
  return await withLock(`git:${resolve(dataDir)}`, async () => {
    ...
    if (!globalRemoteUrl) return; // nothing to sync against
    ...
    try {
      ...
      console.warn("syncGlobalVault: pull failed; aborting any in-progress merge", e);
      try { await git.raw(["merge", "--abort"]); } catch {}
      try { await git.raw(["rebase", "--abort"]); } catch {}
      throw new Error(`Sync failed during pull: ${(e as any)?.message ?? e}`);
    }
    ...
    if (pullSucceeded) {
      ...
      try {
        ...
        console.error("syncGlobalVault: push failed:", error);
...
async function _syncBackupLocked(
  ...
  if (globalRemoteUrl) {
    ...
    try {
      ...
      console.warn("Pull/Merge failed; aborting any in-progress merge and skipping push", e);
